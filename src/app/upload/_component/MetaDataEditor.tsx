...
export default function MetadataEditor({ images }: MetadataEditorProps) {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [tags, setTags] = useState<string[]>(['3D아트', 'Pinz']);
    ...
    <div className="lg:col-span-7 bg-white rounded-[2rem] p-8 sm:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col h-[600px]">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-6 mb-6">
        <div className="bg-slate-100/80 p-1.5 rounded-2xl inline-flex items-center">
          <button className="px-5 py-2 rounded-xl text-xs font-extrabold bg-white text-slate-900 shadow-sm transition-all">
            작품 갤러리
          </button>
          <button className="px-5 py-2 rounded-xl text-xs font-bold text-slate-500 hover:text-slate-900 transition-all">
            커뮤니티 게시판
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col space-y-6 overflow-y-auto no-scrollbar pb-4">
        ...
        <div className="flex-1 min-h-[160px]">
          <textarea
            ...
            placeholder="작품의 비하인드 스토리, 사용한 소프트웨어, 영감을 받은 곳 등 자세한 이야기를 들려주세요..."
      ...
      <div className="pt-6 border-t border-slate-100 flex items-center justify-between mt-auto">
        ...
        <div className="flex gap-3">
          ...
          >
            작품 게시하기 <Rocket className="w-4 h-4" />
          </button>
        </div>
      </div>
    </div>
